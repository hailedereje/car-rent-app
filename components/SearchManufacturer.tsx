    ...
    <div className="search-manufacturer">
      <Combobox value={manufacturer} onChange={setManufacturer}>
        <div className="relative w-full">
          ...
          <Combobox.Button className='absolute top-[14px] cursor-text'>
          ...
          >
            <Combobox.Options>
                ...
                {filterdManufacturer.map((item) => (
                  <Combobox.Option
                    key={item}
                    className={({active})=>`relative search-manufacturer__option text-black ${active ? 'bg-primary-blue text-white' : 'text-gray-900'}`}
                    value={item}
                  >
                    {({active,selected})=> (
                    <>
